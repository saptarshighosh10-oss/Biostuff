...
<script>
const fmt=s=>{s=Math.max(0,Math.round(s));let h=Math.floor(s/3600),m=Math.floor(s%3600/60),x=s%60;return(h?h+'h ':'')+(m?m+'m ':'')+x+'s'};
async function tick(){try{let p=await(await fetch('/progress?x='+Date.now())).json(),now=Date.now()/1000,start=p.started_at||p.file_mtime||now,elapsed=now-start,rate=p.completed/Math.max(elapsed,1),left=(p.total-p.completed)/Math.max(rate,1);pct.textContent=((p.fraction||0)*100).toFixed(2)+'%';bar.value=p.fraction||0;done.textContent=p.completed.toLocaleString();total.textContent=p.total.toLocaleString();document.querySelector('#elapsed').textContent=fmt(elapsed);eta.textContent=p.completed>=p.total?'Complete':fmt(left);updated.textContent='Updated '+new Date((p.updated_at||now)*1000).toLocaleTimeString()}catch(e){updated.textContent='Waiting for progress file...'}}tick();setInterval(tick,2000);
</script>"""
