...
async function tick(){try{let p=await(await fetch('/progress?x='+Date.now())).json(),now=Date.now()/1000; if(!samples.length||samples[samples.length-1].done!==p.completed)samples.push({done:p.completed,time:now}); samples=samples.slice(-6); let first=samples[0],last=samples[samples.length-1],elapsed=now-first.time,rate=(last.done-first.done)/Math.max(last.time-first.time,1),left=(p.total-p.completed)/Math.max(rate,1);pct.textContent=((p.fraction||0)*100).toFixed(2)+'%';bar.value=p.fraction||0;done.textContent=p.completed.toLocaleString();total.textContent=p.total.toLocaleString();document.querySelector('#elapsed').textContent=fmt(elapsed)+' observed';eta.textContent=samples.length<2?'Calculating...':p.completed>=p.total?'Complete':fmt(left);updated.textContent='Updated '+new Date(now*1000).toLocaleTimeString()}catch(e){updated.textContent='Waiting for progress file...'}}tick();setInterval(tick,2000);
</script>"""
